...
func _show_card_rewards() -> void:
	...
	var avaliable_cards: Array[Card] = character_stats.draftable_cards.cards.duplicate(true)
	...
	for i in run_stats.card_rewards:
		...
		for rarity: Card.Rarity in card_rarity_weights:
			if card_rarity_weights[rarity] > roll:
				...
				var picked_card := _get_random_avaliable_card(avaliable_cards, rarity)
				card_reward_array.append(picked_card)
				avaliable_cards.erase(picked_card)
				break
...
func _get_random_avaliable_card(avaliable_cards: Array[Card], with_rarity: Card.Rarity) -> Card:
	var all_possible_cards := avaliable_cards.filter(
		func(card: Card):
